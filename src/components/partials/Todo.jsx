...
function Todo({ todo, setRefreshList }) {
  ...
  return (
    <div className={`mob-todo col-sm-3 mx-3 my-2 alert ${todo.isCompleted ? 'bg-success' : 'bg-light'}`}>
      ...
      <div className='actionButtons my-2' style={{ display: 'flex', justifyContent: "space-between", alignItems: "center" }}>
        <div className='deleteButton'>
          <button style={{ background: 'red', border:"none", padding:"5px",borderRadius:"5px"}} onClick={handleDelete}>Delete</button>
        </div>
        <div className='markTodo my-2'>
          <button onClick={handleMarkTodo} style={{ background: '#6CB4EE',border:"none", padding:"5px" ,borderRadius:"5px"}}>{todo.isCompleted ? "Mark Uncomplete" : "Mark Complete"}</button>
        </div>
      </div>
